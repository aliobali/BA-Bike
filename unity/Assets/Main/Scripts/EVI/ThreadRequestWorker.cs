...
public class ThreadRequestWorker : MonoBehaviour
{
	private string _debugLogPath = Path.Combine(Application.persistentDataPath, "evi_debug_log.json");
	public event Action onNewUpdateArrived;
	...
	public ThreadRequestWorker(string pEVIAdress, double _xOffset, double _yOffset)
	{
		xOffset = _xOffset;
		yOffset = _yOffset;
	...
	private void processMessage(Asmp.Message msgReply)
	{
		if (msgReply.MessageOneofCase == Asmp.Message.MessageOneofOneofCase.Vehicle)
		{
			...
			{
				// Fix mirrored position issue between Unity & Godot
				vehicle.xCoord = -vehicle.xCoord;
	...
	void WorkNetThread()
	{
		UnityEngine.Debug.Log("Starting EVI communication thread with address " + EVIAdress);
		bool running = true;
		...
		while (running)
		{
			try
			{
				if (NewFrameAvailable)
				{
					...
					List<byte[]> multipartResult = new List<byte[]>();
					if (dealerSock.TryReceiveMultipartBytes(TimeSpan.FromMilliseconds(300000), ref multipartResult))
					{
						FrameWasUpdated = true;
						UnityEngine.Debug.Log("Message received from EVI");
						foreach (var resultFrame in multipartResult)
						{
							Asmp.Message msgReply = Asmp.Message.Parser.ParseFrom(resultFrame);
							/*
							string jsonString = JsonFormatter.Default.Format(msgReply);
							File.AppendAllText(_debugLogPath, jsonString + ",\n");
							*/
							UnityEngine.Debug.Log("Process message");
							processMessage(msgReply);
						}
					}
					onNewUpdateArrived?.Invoke();
				}
			}
			catch (ThreadAbortException e)
			{
				...
				UnityEngine.Debug.Log("EVI communication thread aborted due to exception: " + e.ToString());
			}
		}
	}
}