...
namespace Env3d.SumoImporter
{
	public class Extrapolation
	{
		...
		public static void ParseXmlFiles(TextAsset t)
		{
			//TextAsset xmlData = (TextAsset)Resources.Load<TextAsset>(pSumoFilesPath);
			TextAsset xmlData = t;
			...
			using (StringReader rd = new StringReader(xmlData.text))
			{
			...
			Debug.Log($"Junction passed ez: {junctions.Count}");
				...
				if (!edge.functionSpecified)
				{
					...
					foreach (var l in edge.Items)
					{
						if (l is laneType)
						{
							...
							e.AddLane((laneType)l, ref lanes);
						}
					}
				}
			}
		...
		{
			float res = Vector3.Distance(vehicle, junction);
			if (res < 0)
				res = res * -1;
			return res;
		}
		...
		public static VehicleData KeepSpeedAndStayonStreet(double time, VehicleData veh, NetFileLane lane, double xOffset, double yOffset)
		{
			...
			float angle = Vector3.SignedAngle(vspeed, streetSection, Vector3.up);
